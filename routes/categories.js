...
router.get('/countDocument/:url_code', async (req, res) => {
    ...
    try {
        let category = req.params.url_code;
        let data = await CategoryModel.findOne({ url_code: category })

        res.status(200).json({
            count: data.recipe_of_cat_id.length,
            pages: Math.ceil(data.recipe_of_cat_id.length / limit)
            // data
        })
    }
    catch (err) {
...
router.get("/:id", async (req, res) => {
    ...
    try {
        let data = await CategoryModel.findOne({ _id }).populate({
            path: 'recipe_of_cat_id',
            options: {
                skip,
                limit
