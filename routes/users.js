...
router.patch("/role/:id", async (req, res) => {
    try {
        ...
        if (user_id == process.env.SUPERADMIN)
            return res.status(401).json({ msg: "Error You try to change the superadmin!" })
        else {
            let user = await UserModel.updateOne({ _id: user_id }, { role })
            console.log("here");
            let updateUser = await UserModel.findById(user_id)
